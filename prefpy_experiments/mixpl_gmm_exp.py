...
def main(argv):
    ...
    wsse_res = np.empty((p, 4))
    sse_res = np.empty((p, 4))
    time_res = np.empty((p, 10))
    ...
    for n in range(n_init, n_stop + 1, n_step): # for these numbers of agents
        ...
        wsse_vals = np.empty((3,t))
        sse_vals = np.empty((3,t))
        time_vals = np.empty((9,t))
        ...
        for i in range(t):
            ...
            # CUSTOM: top3_full GMM (20 moments)
            ...
            # ULTRA: top3_full GMM (20 moments)
            time_val = time.perf_counter()
            soln, t0, t1 = gmmagg.aggregate(rankings = votes_curr,
                                            ##rankings = None, # for ground-truth empirical limit
                                            algorithm = "top3_full",
            ...
            sse_vals[2][i] = sse_val
            time_vals[6][i] = t0
            time_vals[7][i] = t1
            time_vals[8][i] = time_val
            gmm_result = gmm_mixpl.GMMMixPLResult(num_alts = m,
                                                  num_votes = n,
                                                  ##num_votes = 0, # ground-truth empirical limit
                                                  num_mix = 2,
        ...
        print()
        wsse_res[k_n][0] = n
        wsse_res[k_n][1] = np.mean(wsse_vals[0]) # GMM 1
        wsse_res[k_n][2] = np.mean(wsse_vals[1]) # GMM 2
        wsse_res[k_n][3] = np.mean(wsse_vals[2]) # GMM 3

        sse_res[k_n][0] = n
        sse_res[k_n][1] = np.mean(sse_vals[0]) # EMM 1
        sse_res[k_n][2] = np.mean(sse_vals[1]) # EMM 2
        sse_res[k_n][3] = np.mean(sse_vals[2]) # EMM 3
        ...
        time_res[k_n][4] = np.mean(time_vals[3]) # GMM t0 (moment-calc)
        time_res[k_n][5] = np.mean(time_vals[4]) # GMM t1 (optimization)
        time_res[k_n][6] = np.mean(time_vals[5]) # GMM overall time
        time_res[k_n][7] = np.mean(time_vals[6]) # GMM t0 (moment-calc)
        time_res[k_n][8] = np.mean(time_vals[7]) # GMM t1 (optimization)
        time_res[k_n][9] = np.mean(time_vals[8]) # GMM overall time
