...
from prefpy import evbwie1 as emm
from prefpy_experiments import plot_mixpl_emm3 as plot
np.seterr(all='raise')


def print_usage(argv0):
    print("USAGE: python3", argv0, "<# of alternatives> <# of trials> <# of votes start> <# of votes end> <# votes step> <# iters for EMM 1> <# iters for EM 1> <# iters for EMM 2> <# iters for EM 2> <dataset input filename base> <wsse output filename.csv> <sse output filename.csv> <time output filename.csv> <plot output filename.png> <emm results output filename.p>\n" +
...
def main(argv):
    ...
    #emm_epsilon = 1e-8 # TODO: add option to specify on command-line
    #mm_epsilon = 1e-8 # TODO: add option to specify on command-line
    emm_epsilon = None
    mm_epsilon = None
    tot_iters1 = int(argv[6])
    em_iters1 = int(argv[7])
    tot_iters2 = int(argv[8])
    em_iters2 = int(argv[9])
    #if emm_iters < 1:
    #    emm_epsilon = 1e-8
    #    emm_iters = 1000
    ...
    for n in range(n_init, n_stop + 1, n_step): # for these numbers of agents
        ...
        for i in range(t):
            ...
            emm_pi, emm_p, pi_0, p_0 = emmagg.aggregate(votes_curr,
                                                        K=2,
                                                        epsilon=emm_epsilon,
                                                        tot_iters=tot_iters1,
                                                        epsilon_mm=mm_epsilon,
                                                        max_iters_em=em_iters1
                                                       )
            ...
            emm_result = emm.EMMMixPLResult(num_alts = m,
                                            ...
                                            true_params = params,
                                            epsilon = emm_epsilon,
                                            max_iters = "total:" + str(tot_iters1),
                                            epsilon_mm = emm_epsilon,
                                            max_iters_mm = "actually-EM:" + str(em_iters1),
            ...
            emm_pi, emm_p, pi_0, p_0 = emmagg.aggregate(votes_curr,
                                                        K=2,
                                                        epsilon=emm_epsilon,
                                                        tot_iters=tot_iters2,
                                                        epsilon_mm=mm_epsilon,
                                                        max_iters_em=em_iters2
                                                       )
            time_val = time.perf_counter() - time_val
            soln = np.hstack((emm_pi[0], emm_p[0], emm_p[1]))
            wsse_val = stats.mix2PL_wsse(params, soln, m)
            ...
            emm_result = emm.EMMMixPLResult(num_alts = m,
                                            ...
                                            true_params = params,
                                            epsilon = emm_epsilon,
                                            max_iters = "total:" + str(tot_iters2),
                                            epsilon_mm = emm_epsilon,
                                            max_iters_mm = "actually-EM:" + str(em_iters2),
                                            init_guess = np.hstack((
                                                pi_0[0],
                                                p_0[0],
                                                p_0[1]
        ...
        print()
        wsse_res[k_n][0] = n
        wsse_res[k_n][1] = np.mean(wsse_vals[0]) # EMM 1
        wsse_res[k_n][2] = np.mean(wsse_vals[1]) # EMM 2

        sse_res[k_n][0] = n
        sse_res[k_n][1] = np.mean(sse_vals[0]) # EMM 1
        sse_res[k_n][2] = np.mean(sse_vals[1]) # EMM 2

        time_res[k_n][0] = n
        time_res[k_n][1] = np.mean(time_vals[0]) # EMM 1
    ...
    plot.plot_error_time_data(str_error_type="MSE",
                              emm1_outer=em_iters1,
                              emm1_inner=tot_iters1 // em_iters1,
                              emm2_outer=em_iters2,
                              emm2_inner=tot_iters2 // em_iters2,
                              error_results=sse_res,
                              time_results=time_res,
                              orig_error_results=orig_error_results,
                              orig_time_results=orig_time_results,
                              emm1_error_results=None,
                              emm1_time_results=None,
