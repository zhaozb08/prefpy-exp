...
def main(argv):
    ...
    emm_epsilon = None
    mm_epsilon = None
    emm_iters1 = int(argv[6])
    mm_iters1 = int(argv[7])
    emm_iters2 = int(argv[8])
    mm_iters2 = int(argv[9])
    #if emm_iters < 1:
    #    emm_epsilon = 1e-8
    #    emm_iters = 1000
    ...
    for n in range(n_init, n_stop + 1, n_step): # for these numbers of agents
        ...
        for i in range(t):
            ...
            emm_pi, emm_p, pi_0, p_0 = emmagg.aggregate(votes_curr,
                                                        K=2,
                                                        epsilon=emm_epsilon,
                                                        max_iters=emm_iters1,
                                                        epsilon_mm=mm_epsilon,
                                                        max_iters_mm=mm_iters1
                                                       )
            ...
            emm_pi, emm_p, pi_0, p_0 = emmagg.aggregate(votes_curr,
                                                        K=2,
                                                        epsilon=emm_epsilon,
                                                        max_iters=emm_iters2,
                                                        epsilon_mm=mm_epsilon,
                                                        max_iters_mm=mm_iters2
                                                       )
            time_val = time.perf_counter() - time_val
            soln = np.hstack((emm_pi[0], emm_p[0], emm_p[1]))
            wsse_val = stats.mix2PL_wsse(params, soln, m)
